...
-(void)DEMO:(NSMutableArray *)args
{
    ...
    NSLog(@"\n------------Segger Flasher------------");
    segger = [plist getEquipment:@"Segger"];
    if([segger isReady])
        [segger DEMO];
    
    NSLog(@"\n------------SC18IM700------------");
    scim = [plist getEquipment:@"SCIM"];
    if ([scim isReady])
        [scim DEMO];
    
    NSLog(@"\n------------AARDVARK------------");
    aark = [plist getEquipment:@"AARDVARK"];
    if ([aark isReady])
        [aark DEMO];
    
    NSLog(@"\n------------DFU------------");
    dfu = [plist getEquipment:@"mode_DFU"];
    if ([dfu isReady])
        [dfu DEMO];
    
    NSLog(@"\n------------UDID------------");
    udid = [plist getEquipment:@"mode_UDID"];
    if ([udid isReady])
        [udid DEMO];
    
    NSLog(@"\n------------PWR3615GW------------");
    pwr3615 = [plist getEquipment:@"PWR3615"];
    if ([pwr3615 isReady])
        [pwr3615 DEMO];
    
    NSLog(@"\n------------PWR3631------------");
    pwr3631 = [plist getEquipment:@"PWR3631"];
    if ([pwr3631 isReady])
        [pwr3631 DEMO];
    
    NSLog(@"\n------------RELAY------------");
    relayBox =[plist getEquipment:@"relay"];
    if ([relayBox isReady])
        [relayBox DEMO];
    
    NSLog(@"\n------------METTER------------");
    meter =[plist getEquipment:@"A34401"];
    if ([meter isReady])
        [meter DEMO];
    
    NSLog(@"\n------------Meter USB------------");
    meter_usb =[plist getEquipment:@"MeterUSB"];
    if ([meter_usb isReady])
        [meter_usb DEMO];
    
    NSLog(@"\n------------METTER Client------------");
    meter_clent =[plist getEquipment:@"Mode_Client"];
    if ([meter_clent isReady])
        [meter_clent DEMO];
    
    NSLog(@"\n------------COUNTER------------");
    counter =[plist getEquipment:@"A53131"];
    if ([counter isReady])
        [counter DEMO];
    
    NSLog(@"\n------------ELOAD GPIB------------");
    eload_gpib =[plist getEquipment:@"Eload_ByGPIB"];
    if ([eload_gpib isReady])
        [eload_gpib DEMO];
    
    NSLog(@"\n------------ELOAD USB------------");
    eload_usb =[plist getEquipment:@"Eload_ByUSB"];
    if ([eload_usb isReady])
        [eload_usb DEMO];
    
    NSLog(@"\n------------SCOPE USB------------");
    scope_usb =[plist getEquipment:@"Scope_ByUSB"];
    if ([scope_usb isReady])
        [scope_usb DEMO];
    
    NSLog(@"\n------------USB HID------------");
    hid = [plist getEquipment:@"USBHID"];
    if([hid isReady])
        [hid DEMO];
    
    NSLog(@"\n------------UDP SERVER------------"); // can only select one mode, SERVER or CLIENT
    udp_server = [plist getEquipment:@"udp_Server"];
    if([udp_server isSelected])  // client needs to connent to the server
        [udp_server DEMO];
    
    NSLog(@"\n------------UDP CLIENT------------");
    udp_client = [plist getEquipment:@"udp_Client"];
    if([udp_client isSelected])  // server needs to accept before client connects to the server
        [udp_client DEMO];
    
    NSLog(@"\n------------TCP SERVER------------"); // can only select one mode, SERVER or CLIENT
    tcp_server = [plist getEquipment:@"tcp_Server"];
    if([tcp_server isSelected])  // client needs to connent to the server
        [tcp_server DEMO];
    
    NSLog(@"\n------------TCP CLIENT------------");
    tcp_client = [plist getEquipment:@"tcp_Client"];
    if([tcp_client isSelected])  // server needs to accept before client connects to the server
        [tcp_client DEMO];
    
    NSLog(@"\n------------BOBCAT------------");//it is like FIS but use pudding now
    bobcat = [plist getEquipment:@"BOBCAT"];
    if ([bobcat isReady])
        [bobcat DEMO];
    
    NSLog(@"\n------------STREAM------------");
    bobcat = [plist getEquipment:@"StreamDev"];
    if ([stream isReady])
        [stream DEMO];
