...
#define B207_TIMEOUT 0.05 //change from 0.1(B207) to 0.05 for B238
	...
	// name is an IOKit service name
{
	if ((self = [super init])) {
		...
		[self setReadTimeout:B207_TIMEOUT];
...
- (void)setOptions:(NSDictionary *)newOptions
{
	...
	if ([(NSString *)[newOptions objectForKey:AMSerialOptionServiceName] isEqualToString:[self name]]) {
		...
		[self setSpeed:[temp intValue] isAllowNonStandardBR:NO];
...
- (BOOL)setSpeed:(unsigned long)speed isAllowNonStandardBR:(BOOL)isEnable
{
    cfsetspeed(options, speed);
    int result = tcsetattr(fileDescriptor, TCSANOW, options);//active standard baud rate by TERMIOS API
    
    if (result != 0)
    {
        if (isEnable)
        {
            result = ioctl(fileDescriptor, IOSSIOSPEED, &speed);//active nonstandard baud rate by IOCTL API
            
            if (result != 0)
            {
                NSLog(@"set NonStandard Baud Rates Error");
                return false;
            }
            NSLog(@"set NonStandard Baud Rates Done");
            return true;
        }
        else
        {
            NSLog(@"set Standard Baud Rates Error");
            return false;
        }
    }
    NSLog(@"set Standard Baud Rates Done");
    return true;
}
